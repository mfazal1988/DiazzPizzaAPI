...
namespace DPizza.Infrastructure.Identity.Services
{
    ...
    {
        public async Task<BaseResult<AuthenticationResponse>> RegisterAccount(CreateUserRequest model)
        {
            ...
            var identityUserResult = await userManager.CreateAsync(user, model.Password);

            if (identityUserResult.Succeeded)
            {
                var identityRoleResult = await userManager.AddToRoleAsync(user, model.Role);
                ...
                {
                    return new BaseResult<AuthenticationResponse>(identityRoleResult.Errors.Select(p => new Error(ErrorCode.ErrorInIdentity, p.Description)));
                }
            }

            return new BaseResult<AuthenticationResponse>(identityUserResult.Errors.Select(p => new Error(ErrorCode.ErrorInIdentity, p.Description)));
        }
