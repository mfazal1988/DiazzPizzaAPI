...
{
    public class UpdateOrderDeliveryPersonCommandHandler(IOrderRepository orderRepository, IUnitOfWork unitOfWork, ITranslator translator) : IRequestHandler<UpdateOrderDeliveryPersonCommand, BaseResult>
    {
        public async Task<BaseResult> Handle(UpdateOrderDeliveryPersonCommand request, CancellationToken cancellationToken)
        {
            ...
            order.UpdateOrderDeliveryPerson(request.Id,  request.DeliveryPerson);
